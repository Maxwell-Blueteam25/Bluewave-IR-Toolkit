...
function Invoke-Sweep {
    ...
    $RawJsonContent = Get-Content $InputJson -Raw
    $Schema = "sweeper-schema.json"
    ...
    if (-not (Test-Json -Json $RawJsonContent -SchemaFile $Schema)){
        ...
        Write-Host "Config is Valid. Ready For Deployment." -ForegroundColor Green
    }
    ...
    foreach ($SweepItem in $ConfigObject.sweeps) {
        ...
        if ($SweepItem.type -eq "registry_value"){
            ...
            Write-Host "Checking Registry Key: $($SweepItem.value)" -ForegroundColor Yellow
            ...
            Write-Host "Checking Process: $($SweepItem.value)" -ForegroundColor Yellow
